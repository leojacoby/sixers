...
ggplot(data = lower_level) + 
  geom_histogram(aes(x = Seat_comfort_satisfaction, y = ..density..), stat = "count", binwidth = 1) +
...
sound_results <- results %>% 
  filter(Sound_satisfaction %in% c("About right", "Too loud")) %>% 
  ...
  mutate(too_loud_percent = (age_total - n)/age_total) %>% 
  filter(Sound_satisfaction == "About right") %>% 
  arrange(Age)

sound_results$Age <- c(21, 27, 32, 37, 42, 47, 52, 57, 62, 67, 72)





fit <- lm(too_loud_percent ~ Age, data = sound_results)
...
ggplot(data = sound_results) +
  ...
  labs(y = "Portion of fans who think audio is too loud", title = "Fan age vs. opinion on arena audio") +
  NULL

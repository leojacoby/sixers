...
results <- results %>% 
  ...
  mutate(Games = as.numeric(Games),
         Upgrade_app_freq = ifelse(
           !(grepl("Frequently", Upgrade_app_freq) | grepl("Sometimes", Upgrade_app_freq)), 
           "Never", 
           Upgrade_app_freq))
...
results$Upgrade_app_freq <- factor(results$Upgrade_app_freq, 
                                 ordered = TRUE,
                                 levels = c("Never", "Sometimes", "Frequently"))

content_model <- polr(formula = Upgrade_app_freq ~ Age + Games, data = results, Hess = TRUE)

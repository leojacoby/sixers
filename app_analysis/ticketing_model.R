...
results <- results %>% 
  ...
  mutate(Games = as.numeric(Games),
         Ticket_app_freq = ifelse(
           !(grepl("Frequently", Ticket_app_freq) | grepl("Sometimes", Ticket_app_freq)), 
           "Never", 
           Ticket_app_freq))
...
results$Ticket_app_freq <- factor(results$Ticket_app_freq, 
                                               ordered = TRUE,
                                               levels = c("Never", "Sometimes", "Frequently"))

ticketing_model <- polr(formula = Ticket_app_freq ~ Age + Games, data = results, Hess = TRUE)

ctable <- coef(summary(ticketing_model))

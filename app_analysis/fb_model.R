...
results <- results %>% 
  ...
  mutate(Games = as.numeric(Games),
         FB_app_freq = ifelse(
           !(grepl("Frequently", FB_app_freq) | grepl("Sometimes", FB_app_freq)), 
           "Never", 
           FB_app_freq))
...
results$FB_app_freq <- factor(results$FB_app_freq, 
                                   ordered = TRUE,
                                   levels = c("Never", "Sometimes", "Frequently"))

content_model <- polr(formula = FB_app_freq ~ Age + Games, data = results, Hess = TRUE)

...
results <- results %>% 
  ...
  mutate(Games = as.numeric(Games),
         Merch_app_freq = ifelse(
           !(grepl("Frequently", Merch_app_freq) | grepl("Sometimes", Merch_app_freq)), 
           "Never", 
           Merch_app_freq))
...
results$Merch_app_freq <- factor(results$Merch_app_freq, 
                                   ordered = TRUE,
                                   levels = c("Never", "Sometimes", "Frequently"))

content_model <- polr(formula = Merch_app_freq ~ Age + Games, data = results, Hess = TRUE)
